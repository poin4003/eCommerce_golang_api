...
func (q *Queries) GetTwoFactorMethodByIDAndType(ctx context.Context, arg GetTwoFactorMethodByIDAndTypeParams) (PreGoAccUserTwoFactor9999, error) {
	row := q.db.QueryRowContext(ctx, getTwoFactorMethodByIDAndType, arg.UserID, arg.TwoFactorID)
	var i PreGoAccUserTwoFactor9999
	err := row.Scan(
		&i.TwoFactorID,
		&i.UserID,
...
const verifyTwoFacto = `-- name: VerifyTwoFacto :one
SELECT COUNT(*)
FROM pre_go_acc_user_two_factor_9999
WHERE user_id = ? AND two_factor_auth_type = ? AND two_factor_is_active = TRUE
`

type VerifyTwoFactoParams struct {
	UserID            int32
	TwoFactorAuthType PreGoAccUserTwoFactor9999TwoFactorAuthType
}

// VerifyTwoFactor
func (q *Queries) VerifyTwoFacto(ctx context.Context, arg VerifyTwoFactoParams) (int64, error) {
	row := q.db.QueryRowContext(ctx, verifyTwoFacto, arg.UserID, arg.TwoFactorAuthType)
	var count int64
	err := row.Scan(&count)
	return count, err
}
